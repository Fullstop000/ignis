...
|------|-----------|----------------|-------|-------|----------:|--------:|----------:|-----------:|----------:|-----------:|--------|-------|
| 2026-06-08 | Terminal-Bench 2.1 (89) | `zhipu/glm-5.1` | v0.37.0 † | 18/89 · 20.2% | 20.2% | 0 | 25.6M | 0.87M | 88.3% | n/a | [csv](terminal-bench/history/tb21-glm-5.1-20260608.csv) | **Account billing quota hit mid-run — headline number is not a capability signal.** At 20:14:10 UTC (trial 32/89, `make-doom-for-mips`), the BigModel account started returning `code: 1113 余额不足或无可用资源包,请充值` (insufficient balance) on every request; 55 of the remaining 58 trials failed instantly with no model attempt. The aggregator can't pattern-match the Chinese billing error so it bucketed all 55 as `failed` (reward 0 + empty exception) — they belong conceptually in `errored` as a new `BalanceExhausted` sub-type. **Fair-shot capability on the 31 pre-quota trials: 15/31 = 48.4%** — competitive with TB 2.1 baselines (MiniMax-M3 47–52%, deepseek-v4-flash@max 56.2%). Within the fair-shot subset: 15 passed + 16 capability-failed (3 `TimedOut` + 13 verifier-reject). 0 errored per the raw CSV bucketing (RateLimited / ConnectionDropped did not fire — quota error came back as a normal 200 with error JSON, not a 429). Est. cost `n/a` pending a BigModel pricing entry in footnote ‡. Plan: top up the account, re-run the 55 quota-failed slugs to produce a clean row. |
