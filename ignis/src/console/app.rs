    ...
    pub(crate) fn handle_event(&mut self, ev: AgentEvent) {
        match ev {
            ...
            AgentEvent::Notice { message } => {
                // Neutral out-of-band line (no `[warn]` framing). Native's own
                // `/connect` uses `add_assistant_notice` directly, so this only
                // fires if some engine-side path emits a Notice — render it the
                // same way.
